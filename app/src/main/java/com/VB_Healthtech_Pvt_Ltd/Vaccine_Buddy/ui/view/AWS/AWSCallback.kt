...
import com.catalyist.aws.AWSConstants.IMAGE_URL
import java.io.File
import kotlin.math.roundToInt

class AWSCallback(var image: File?, var listner : AWSListner?, var folder_path: String): TransferListener {
...
//                TransferState.COMPLETED -> { listner?.onAWSSuccess(IMAGE_URL+image?.name) }
                TransferState.COMPLETED -> { listner?.onAWSSuccess(IMAGE_URL.plus(folder_path)) }
